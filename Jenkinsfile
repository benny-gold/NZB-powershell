pipeline {
  ...
  stages {
    ...
    stage('Run Tests') {
      steps {
        bat '"C:\\Windows\\SysNative\\WindowsPowerShell\\v1.0\\Powershell.exe" -ExecutionPolicy ByPass -noprofile -command "[Environment]::Is64BitProcess;md reports -ea SilentlyContinue;Invoke-Pester -OutputFormat NUnitXml -OutputFile .\\reports\\$($env:BUILD_NUMBER)_Tests.xml"'
